...
import requests, time, html, humanize, operator, re
...
class Mangadex(MangaSource):
    ...
    def top(self, count=5):
        soup = BeautifulSoup(requests.get(self.root_url, headers={'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.149 Safari/537.36 Edg/80.0.361.69'}).text, features='html.parser')
        mgd = []
    ...
    def updates(self, count=5):
        soup = BeautifulSoup(requests.get(self.update_url, headers={'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.149 Safari/537.36 Edg/80.0.361.69'}).text, features='html.parser')
        mgd = []
